...
class MessageLayer(object):
    ...
    def handle_message(self, message):
        ...
        if t is None:
            logging.error(defines.types[message.type] + " received without the corresponding message")
            return
        ...
        if message.type == defines.inv_types['RST']:
            for resource in self._parent.relation.keys():
                ...
                if observers is not None:
                    del observers[key]
                    logging.info("Cancel observing relation")
                    if len(observers) == 0:
                        del self._parent.relation[resource]

        # cancel retransmission
        logging.info("Cancel retrasmission to:" + host + ":" + str(port))
        try:
